...
function installKicad
{
    ...
    desired_kicad_major="8"
    echo "Ubuntu 25.04 detected. Using KiCad 8 from the Ubuntu universe repository."
    ...
    if dpkg -s kicad &>/dev/null; then
        ...
        if [[ "$installed_version" != "$desired_kicad_major" ]]; then
            echo "A different version of KiCad ($installed_version) is installed."
            read -p "Do you want to remove it and install KiCad $desired_kicad_major.x? (yes/no): " response
            ...
            echo "KiCad $desired_kicad_major.x is already installed. Verifying support packages..."
        fi
    fi
...
function installDependency
{
    ...
    sudo apt-get install -y python3-venv python3-pip python3-setuptools python3-psutil python3-pyqt5 python3-matplotlib xterm
